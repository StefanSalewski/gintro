...
let nims = """
int int8 int16 int32 int64 uint uint8 uint16 uint32 uint64 float float32 float64 pointer
"""
...
proc writeObj(info: GIObjectInfo) =
  ...
  for j in 0 .. <numsig:
    ...
    output.writeLine(" p: proc (self: ptr gobject.Object00; " & h & " {.cdecl.}, xdata: pointer = nil) =")
    output.write("  discard g_signal_connect_data(self.impl, \"")
...
proc main(namespace: string) =
  ...
  let Lib = if libs.isNil: "" else: ($libs).split(',', 2)[0]
