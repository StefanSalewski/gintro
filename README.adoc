...
NOTE: We fixed a few small bugs in the last weeks, so we decided to finally make a 0.8.0 release.
The GTK4 examples from http://ssalewski.de/gtkprogramming.html
should work now, and the existing GTK3 examples seems still to work. This release is already available since a few weeks by
*"nimble install gintro@#head"* command, so we assume that there are no serious issues.
We have added a simple VTE example to the GTK3 example directory.
If you are using Arch Linux you may still have to ensure that
