...
NOTE: This is version 0.7.9 which contains again some changes. The two GTK4 examples from http://ssalewski.de/gtkprogramming.html
should work now, and the existing GTK3 examples seems still to work. Unfortunately there is a tiny bug in this release when you
are using VTE module, see https://github.com/StefanSalewski/gintro/issues/82. As only very few people are using VTE we will
not make a new release to fix this issue, so please use the comand *"nimble install gintro@#head"* in this case to get latest fix.
We have added a simple VTE example to the GTK3 example directory. If you are using Arch Linux you may have to ensure that
