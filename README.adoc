...
With the latest fix to Nim  GTK3 bingings it seems to work fine, but some rewriting for optimized performance
may be needed, I have to investigate that.

NOTE: The latest needed gintro patch is not yet available, I will provide it soon...
...
proc drawWorld(cr: cairo.Context) =
  ...
  var i = 10.0
  while true:
    if [DataWidth - 2.0 * i, DataHeight - 2.0 * i].min <= 0:
      break
...
proc updateVal(adj: PosAdj; d: float) =
  ...
  adj.setValue([0.0, [adj.value + d, adj.upper - adj.pageSize].min].max)
...
proc dareaConfigureCallback(darea: DrawingArea; event: EventConfigure;
  ...
  this.fullScale = [this.darea.allocatedWidth.float / this.dataWidth,
      this.darea.allocatedHeight.float / this.dataHeight].min
  this.paint
...
proc scrollEvent(darea: DrawingArea; event: EventScroll; this: PDA): bool =
  ...
  var d: ScrollDirection
  discard scrollDirection(event, d)
  var x, y: float
  discard event.getCoords(x, y)
    ...
    this.updateAdjustmentsAndPaint((this.hadjustment.value +
        this.darea.allocatedWidth.float * 0.5) * (this.userZoom / z0 - 1),
                                 (this.vadjustment.value +
                                     this.darea.allocatedHeight.float * 0.5) * (this.userZoom / z0 - 1))
...
proc buttonReleaseEvent(darea: DrawingArea; event: EventButton;
  ...
  discard event.getCoords(x, y)
  var b: int
  discard button(event, b)
  if b == 1:
    this.selecting = false
    let z1 = [this.darea.allocatedWidth.float / (
        this.lastButtonDownPosX - x).abs, this.darea.allocatedHeight.float / (
        this.lastButtonDownPosY - y).abs].min
...
proc newPDA: PDA =
  ...
  result.attach(result.darea, 0, 1, 0, 1, {AttachFlag.expand, AttachFlag.fill}, {
      AttachFlag.expand, AttachFlag.fill}, 0, 0)
  result.attach(result.hscrollbar, 0, 1, 1, 2, {AttachFlag.expand,
      AttachFlag.fill}, {}, 0, 0)
  result.attach(result.vscrollbar, 1, 2, 0, 1, {}, {AttachFlag.expand,
      AttachFlag.fill}, 0, 0)
